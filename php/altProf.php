<?php
	...
	$args = array($nome, $senha, $confirmar, $nascimento, $genero);
	...
	if ($valido == true){
		...
		$query = "UPDATE `professor` SET `senha`=?,`nome`=?,`nascimento`=?,`genero`=? WHERE `cpf`=?";
		$stmt = $conn->prepare($query);
		$stmt->bind_param("ssssi", $senhaHash, $nome, $nascimento, $genero, $_SESSION['cpf']);
		$stmt->execute();
	}else{
		return;
	}
